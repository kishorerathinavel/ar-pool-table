...
int exposure_micro = 16000;
float gain_db = 14;
int downsample = 2;
int pixel_skip = 0; //skip pixels, rather than bin
int pixel_format = XI_RGB24;// XI_MONO8,XI_RAW8

//ximea calibration
float intrinsicsScaleFactor = 1.0; 	//scale intrinsics values by this amount (e.g. if intrinsics computed using different resolution)
float poolImgPlaneWidth = 2592*0.0022;	//sensor size
float poolImgPlaneHeight = 1944*0.0022;
float focalLength = 3.82925415;
int poolImageWidth = 648*2, poolImageHeight = 486*2;
float imageCenter[2] = {649.430481,poolImageHeight-(460.071228)}; //calibrated camera center, adjusted so (0,0) is bottom left corner
int scaleFactor = 1; //perform checkerboard detection at 1/scaleFactor resolution

float whiteBalance[3] = {0.92,0.78,1.45};

//misc
long poolCurrentFrame = 0;
struct timeval poolILasttime;
int poolScreenWidth, poolScreenHeight;
...
//checkerboard
float square_size = 3.75;
int corner_rows = 6, corner_cols = 4;
...
float greenFeltMinVal = 60;
float greenFeltMaxVal = 80;
float lineWidth = 16;
...
void saveHSVImage() {
  IplImage *hsvImg;
  hsvImg = cvCreateImage(cvSize(poolScreenWidth,poolScreenHeight),IPL_DEPTH_8U,3);
...
void setup3DDraw() {
    ...
    //float aspect = (float)poolScreenWidth/poolScreenHeight;
    //gluPerspective(fov/aspect, aspect, 1, 1000);
    float w = poolImgPlaneWidth/focalLength*near, h = poolImgPlaneHeight/focalLength*near;
    glFrustum(-imageCenter[0]/poolImageWidth*w, (poolImageWidth-imageCenter[0])/poolImageWidth*w, -imageCenter[1]/poolImageHeight*h,(poolImageHeight-imageCenter[1])/poolImageHeight*h, near, far); 
    //glFrustum((poolImageWidth-imageCenter[0])/poolImageWidth*w, -imageCenter[0]/poolImageWidth*w,  (poolImageHeight-imageCenter[1])/poolImageHeight*h, -imageCenter[1]/poolImageHeight*h, near, far); //flip l,r and b,t to account for camera being upside down

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    poolExtrinsicsTransform();
...
void detectCalibrationPattern() {
  ...
  if(useCircleDetector) {	//circles
	...
	//disabling for now, doesn't work well for checkerboard at steep angle
...
void calculateShot() {
  ...
  H = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,1);
  S = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,1);
  V = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,1);
  ...
  inRange(oH1, Scalar(greenFeltMinVal), Scalar(greenFeltMaxVal), mH1);//AM modified
  //namedWindow( "Stage 2", WINDOW_AUTOSIZE );
  //imshow("Stage 2", mH1);
  //waitKey();
  ...
  //namedWindow( "Stage 5", WINDOW_AUTOSIZE );
 //  imshow("Stage 5", mH2);
 //  waitKey();
  // if(first) 
  //   imwrite("./outputs/roi.jpg", mH2);
  inRange(mH2, Scalar(0), Scalar(greenFeltMinVal-5), mH3);//AM modified
  bitwise_and(mH1, mH3, mH3);
  // imshow("Stage 6", mH3);
  inRange(mH2, Scalar(greenFeltMaxVal+5), Scalar(255), mH4);//AM modified
  bitwise_or(mH3, mH4, mH5);
  morphologyEx(mH5, mH5, MORPH_CLOSE, Mat(), Point(-1, -1), 4);
  medianBlur(mH5, mH5, 3);
//namedWindow( "Stage 7", WINDOW_AUTOSIZE );
//  imshow("Stage 7", mH5);
//waitKey();
...
void simShot() {
  ...
  maxCount = sqrt(sqDiff.at<int>(0,0))/2000;
...
void poolUpdate() {
 ...
 //get image
  ...
  if(standalone) {
	  ...
	  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, poolScreenWidth/scaleFactor, poolScreenHeight/scaleFactor, 0, GL_BGR, GL_UNSIGNED_BYTE, img->imageData);
...
void updateWhiteBalance() {
	stat = xiSetParamFloat(xiH, XI_PRM_WB_KR, whiteBalance[0]);
	HandleResult(stat,"xiSetParam (white balance red)");

	stat = xiSetParamFloat(xiH, XI_PRM_WB_KG, whiteBalance[1]);
	HandleResult(stat,"xiSetParam (white balance green)");

	stat = xiSetParamFloat(xiH, XI_PRM_WB_KB, whiteBalance[2]);
	HandleResult(stat,"xiSetParam (white balance blue)");

}



void poolKeyPressed(unsigned char key, int x, int y) {
 
  switch(key) {
	  case 27: if (xiH) xiCloseDevice(xiH); exit(0); break;
	  ...
	  case '7': whiteBalance[0] -= 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
	  case '9': whiteBalance[0] += 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
	  case '4': whiteBalance[1] -= 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
	  case '6': whiteBalance[1] += 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
	  case '1': whiteBalance[2] -= 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
	  case '3': whiteBalance[2] += 0.01; updateWhiteBalance(); printf("white bal %f %f %f\n", whiteBalance[0], whiteBalance[1], whiteBalance[2]); break;
  }
}

void poolKey(unsigned char key) {
	poolKeyPressed(key,0,0);
}
...
void poolInit(const char * configPath) {
  ...
  poolScreenWidth = (int)image.width; 
  poolScreenHeight = (int)image.height;

 //create cv images
  imgOrig   = cvCreateImage(cvSize(poolScreenWidth,poolScreenHeight),IPL_DEPTH_8U,3);
  imgResize = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,3);
  img       = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,3);
  imgGray   = cvCreateImage(cvSize(poolScreenWidth/scaleFactor,poolScreenHeight/scaleFactor),IPL_DEPTH_8U,1);

  char temp[1024];
  snprintf(temp, 1024, "%scam_rgb_0.xml", configPath);
  cam_mat = (CvMat*)cvLoad(temp, NULL, NULL, NULL);
  snprintf(temp, 1024, "%sdistort_rgb_0.xml", configPath);
  dist_coeff = (CvMat*)cvLoad(temp,  NULL, NULL, NULL);
  ...
  snprintf(temp, 1024, "%sextrinsics.xml", configPath);
  extrinsics = (CvMat*)cvLoad(temp, NULL, NULL, NULL);
  if(extrinsics == NULL) {
    ...
    snprintf(temp, 1024, "%spocketPositions.xml", configPath);
    FileStorage file(temp, FileStorage::READ);
    file["pocketPositions"] >> pocketPositions;
    file.release();
...
void glutDisplay() {
  ...
  if(poolCurrentFrame%printNthFrame==0&&poolCurrentFrame!=0) {
    struct timeval now;
    gettimeofday(&now, NULL);

    double elapsed_sec = (now.tv_sec-poolILasttime.tv_sec)+(now.tv_usec-poolILasttime.tv_usec)/1000000.0;
    printf("avg fps: %f\n", printNthFrame/elapsed_sec); 

    poolILasttime.tv_sec = now.tv_sec;
    poolILasttime.tv_usec = now.tv_usec;
  }
  poolCurrentFrame++; 
...
int main(int argc, char** argv) {
  ...
  glutInitWindowSize(poolScreenWidth,poolScreenHeight);
  glutCreateWindow ("Pool Demo");
  glutDisplayFunc(glutDisplay); 
  glutKeyboardFunc(poolKeyPressed);
  glutMouseFunc(poolMouseButton);
