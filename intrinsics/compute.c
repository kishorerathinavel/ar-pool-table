...
int main (int argc, char **argv)
{
  ...
  CvPoint2D32f* corners = malloc(sizeof(CvPoint2D32f)*corner_rows*corner_cols);
  for(type = 0; type <= 1; type++) {
	...
	//load image
	snprintf(temp, 1024, "%s/img_%s_%d.png", intDir, type==0?"rgb":"ir", j*skip);
	printf("loading %s...\n",temp);
	...
	point_counts = point_counts2;
      ...
      //compute intrinsics
      double error = cvCalibrateCamera2(object_points, image_points, point_counts, cvSize(width, height), cam_mat, dist_coeff, 
					NULL, NULL, 0/*CV_CALIB_FIX_K3*/, cvTermCriteria(CV_TERMCRIT_ITER+CV_TERMCRIT_EPS,30,DBL_EPSILON) ); //possibly use CV_CALIB_FIX_K3 for last param if not a lot of distortion
      ...
      //save matrices
      snprintf(temp, 1024, "%s/cam_%s_%d.xml", outDir, type==0?"rgb":"ir", i);
      cvSave(temp, cam_mat, NULL, NULL, cvAttrList(NULL, NULL));
      snprintf(temp, 1024, "%s/distort_%s_%d.xml", outDir, type==0?"rgb":"ir", i);
